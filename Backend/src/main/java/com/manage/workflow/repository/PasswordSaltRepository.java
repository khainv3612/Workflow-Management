...
public interface PasswordSaltRepository extends JpaRepository<PasswordSalt,Long> {
    @Query(value = "SELECT p.passwordSalt from PasswordSalt p JOIN (SELECT a.id FROM Account a WHERE a.username=:username) t ON p.idAccount = t.id",nativeQuery = true)
